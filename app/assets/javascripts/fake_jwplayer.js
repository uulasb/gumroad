(function () {
  ...
  function makePlayer(elOrId) {
    ...
    const api = {
      ...
      once(evt, cb) { (onceHandlers[evt] ||= []).push(cb); },
      getState() { return state; },
      getPosition() { return position; },
      getDuration() { return duration; },
      getPlaylistItem() { return item; },
      __armAutoResume() { shouldAutoResume = true; },
      __debugState() { return { state, position, item, shouldAutoResume }; },
      load(arr) {
        state = 'idle';
      ...
      seek(pos) { position = Math.max(0, Math.min(duration - 0.5, pos)); },
      play() {
        window.__JW_TRACE__.push('play()');
        if (window.__EXPIRE_HLS__ === true) {
          // Simulate signed URL expiry surface (403/410)
          setTimeout(() => { window.__JW_TRACE__.push('emit(error 403)'); emit('error', { code: 403 }); }, 50);
          return;
        }
